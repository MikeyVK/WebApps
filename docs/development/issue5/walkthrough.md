...
- **Geïntegreerde Klikbare Bullets & Blok-checks**: Criteria en selectievakjes zijn volledig geïntegreerd in één strakke lijst van klikbare bullets. De statische opsomming en de losse checkboxen zijn samengevoegd, waardoor dubbele teksten en herhalingen volledig zijn geëlimineerd. Tevens is de status-checkbox in de header van elke kaart klikbaar gemaakt: hiermee kan de gebruiker in één klik alle onderliggende criteria binnen dat blok tegelijk in- of uitschakelen.
...
- **Eenduidige Uitlijning, Rescaling & Print Checkbox Fix**:
  - De checklist-items worden op desktop in een nette, responsieve 2-koloms grid (`grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2`) getoond naast het icoon, waardoor de beschikbare breedte optimaal wordt benut. Dit maakt de kaarten veel compacter en elimineert onnodige verticale witruimte.
