...
- **Uitlijning & Balans Fix**:
  - Kolomverhoudingen binnen de kaarten zijn vastgelegd op een gebalanceerde verdeling: `flex-[3]` voor de afbeeldingen/bullet points en `flex-[2]` voor de checkboxes, wat overmatig in elkaar drukken van de tekst voorkomt.
  - Uitlijning is veranderd van `items-center` naar `items-start` op zowel de kaartrijen als de linkerkolom, zodat de iconen en checklists netjes bovenaan de tekst uitlijnen.
